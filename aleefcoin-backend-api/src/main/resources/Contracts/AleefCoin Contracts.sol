...
contract Aleefcoin is SafeMath,owned{
    string public name;
    ...
    /*This notifies clients about the amount for Approval*/
	...
	function freeze(address _freeze,uint256 _value) onlyOwner returns (bool success) {
        if (balanceOf[_freeze] < _value) revert();            // Check if the sender has enough
	    if (_value <= 0) revert(); 
	...
	function unfreeze(address _unfreeze,uint256 _value) onlyOwner returns (bool success) {
        if (freezeOf[_unfreeze] < _value) revert();            // Check if the sender has enough
		if (_value <= 0) revert(); 
	...
	function transferCrowdsale(address to, uint256 value) {
